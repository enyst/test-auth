...
async def test_app():
    ...
    async with httpx.AsyncClient() as client:
        ...
        # Test GitHub endpoints (with token)
        print("5. Testing GitHub repos endpoint (with token)...")
        try:
            headers = {"X-GitHub-Token": "your_token_here"}
            response = await client.get(f"{base_url}/api/v1/github/repos", headers=headers)
            print(f"   ✅ Status: {response.status_code}")
